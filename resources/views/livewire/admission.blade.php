    ...
    <form wire:submit.prevent="admit">
        <div class="mb-4">
            <select wire:model.lazy="lead_id" class="lms-input">
                ...
                <option value="{{$lead->id}}">Name: {{$lead->name}} | Email: {{$lead->email}} | Phone: {{$lead->phone}}
                </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            <input wire:model.lazy="payment" type="number" step=".01" max="{{number_format($selectedCourse->price, 2)}}"
                class="lms-input" placeholder="Payment now">
        </div>
